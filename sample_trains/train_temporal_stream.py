...
if __name__ == '__main__':
    ...
    videos_dir = 'G:\\images\\videos'
    model_save_path = 'G:\\models\\temporal_stream_model'
    num_class = 3142
    has_train = False
    has_eval = True
    ...
    # The dataset
    if has_train:
        # dataset = dt.VideoDataset(csv_file, class_csv_file, videos_dir, 3142, parted=True, num_parts=2)
        dataset = dtv.VideoDataset(csv_file, class_csv_file, videos_dir, 3142, parted=False)
        dataloader = DataLoader(dataset, batch_size=8, shuffle=True, num_workers=4)
        # print(len(dataloader))
    if has_eval:
        # dataset_eval = dt.VideoDataset(csv_file_eval, class_csv_file, videos_dir, 3142, parted=True, num_parts=2)
        dataset_eval = dtv.VideoDataset(csv_file_eval, class_csv_file, videos_dir, 3142, parted=False)
        dataloader_eval = DataLoader(dataset_eval, batch_size=8, shuffle=True, num_workers=4)
        # print(len(dataloader))

    # The model
    model = tm.SlowFusionVideoModelSharedWeightsFC(3, 3142)
    model.to(device)
    ...
    for epoch in range(num_epochs):
        ...
        if has_eval:
            ...
            print(
                f'Epoch [{epoch + 1}/{num_epochs}], Evaluation Metric: {eval_metric:.4f}, Validation Loss: {val_loss:.4f}')
