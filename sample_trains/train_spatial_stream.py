...
if __name__ == '__main__':
    ...
    images_dir = 'G:\\images\\images'
    model_save_path = 'G:\\models\\spatial_stream_model'
    num_class = 3142
    has_train = False
    has_eval = True
    ...
    # The dataset
    if has_train:
        dataset = dt.ImageDataset(csv_file, class_csv_file, images_dir, 3142, parted=False)
        # dataset = dtv.ImageDataset(csv_file, class_csv_file, images_dir, 3142, parted=True, num_parts=2)
        dataloader = DataLoader(dataset, batch_size=8, shuffle=True, num_workers=4)
        # print(len(dataloader))
    if has_eval:
        dataset_eval = dt.ImageDataset(csv_file_eval, class_csv_file, images_dir, 3142, parted=False)
        # dataset_eval = dtv.ImageDataset(csv_file_eval, class_csv_file, images_dir, 3142, parted=True, num_parts=1)
        dataloader_eval = DataLoader(dataset_eval, batch_size=8, shuffle=True, num_workers=4)
        # print(len(dataloader))

    # The model
    model = sm.ResNet(3142)
    model.to(device)
